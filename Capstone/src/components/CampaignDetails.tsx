...
const CampaignDetails: React.FC<{ campaign: Campaign }> = ({ campaign }) => {
  const goalReachedText =
    campaign.amtRaised >= campaign.goal ? 'Reached!' : 'Not Reached!'
  const goalReachedColor =
    campaign.amtRaised >= campaign.goal ? 'text-red-600' : 'text-yellow-600'
  ...
  return (
    <div className="md:col-span-2">
      ...
      <p className="text-gray-600 leading-relaxed">{campaign?.desc}</p>
      ...
      <div className="mt-6">
        ...
        <div className="w-full bg-gray-300 rounded-lg h-4">
          <div
            ...
            style={{
              width: `${(campaign?.amtRaised / campaign?.goal) * 100}%`,
            }}
          />
        </div>
        <p className="mt-2 text-gray-700">
          {campaign?.amtRaised.toLocaleString()} SOL raised of{' '}
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
            <img src="/jito.png" alt="JITO" className="h-6 w-6 rounded-full" />
            JitoSOL Withdrawals
          </h3>
          <p className="text-gray-700">
